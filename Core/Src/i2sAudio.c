...
{
  i2sHandle = hi2s;
  systemStatus.file = &SDFile;
  systemStatus.fat = &SDFatFS;
}
...
void handleAudio(void)
{
  ...
  if (systemStatus.audioStatus == audio_play)
  {
    ...
    char btnNow = HAL_GPIO_ReadPin(KEY1_GPIO_Port, KEY1_Pin);
    if (!btnNow && btnPrev && HAL_GetTick() - btnTimer > 200)
    { // If button pressed for more than 200ms, force end of file to skip it and play next one
      AudioStop();
      systemStatus.fileStatus = file_end;
    }
    if (btnNow != btnPrev)
    { // If button reading different than last reading, reset the timer and store value
      btnNow = btnPrev;
...
{
  if (systemStatus.PCMbuffer || systemStatus.decoder)
  {              // If not empty, something went out of control
...
//    break;
  ...
  default:
    printf("AUDIO: Skipping unknown filetype!\n");
    AudioStop();
    return;
  }
  ...
  if (systemStatus.remainingSamples == 0)
  {              // If zero bytes transferred or error
    AudioStop(); // Stop
    return;
  }
  else
  {
    if (systemStatus.remainingSamples < systemStatus.PCMSamples)
    {                                                                                                                                // If less than full buffer size transferred
...
void handleBuffer(uint16_t offset)
{
  ...
  { // File already reached end, so no more data to transfer
    if (systemStatus.remainingSamples <= systemStatus.PCMSamples / 2)
    {              // Remaining bytes less than 1/2 buffer?
